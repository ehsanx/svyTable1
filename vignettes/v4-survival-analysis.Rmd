...
The multiple-imputation pipeline relies on `mitools::imputationList()`, so this
chunk only runs when the suggested **mitools** package is installed.

```{r mi, eval = requireNamespace("mitools", quietly = TRUE)}
data(nhanes_mortality)
...
allImputations_main <- mitools::imputationList(split(impdata_long, impdata_long$.imp))
...
allImputations_split <- mitools::imputationList(split(impdata_long_split, impdata_long_split$.imp))
