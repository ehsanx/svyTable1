...
## 7. Visualizing Interaction Effects with `plotint`

While the functions `addintlist()` and `reportint()` are powerful for generating tables of additive and multiplicative interaction for categorical variables, a common challenge is visualizing interactions involving continuous variables.

The `plotint()` function is a general-purpose tool designed to solve this. It uses the "simple slopes" (or "pick-a-point") methodology [@aiken1991multiple; @preacher2006computational] to probe and plot interactions from `svyglm`, `glm`, `svycoxph`, and `coxph` models.

It can handle three main scenarios:

- **Continuous × Continuous:** (e.g., Age × BMI)  
- **Continuous × Categorical:** (e.g., Age × Sex)  
- **Categorical × Categorical:** (e.g., Obesity × Race)

The function generates a `ggplot2` object, allowing you to visualize how the effect of a primary exposure (*effect*) changes across different levels of a moderator (*moderator*).

---

### Example: Continuous × Continuous Interaction (NHANESraw)

Let's examine if the effect of Age on blood pressure is modified by BMI. Both are continuous variables.

#### 1. Data Preparation and Model Fitting

We first load the NHANESraw data, filter for a complete-case dataset, and fit the model. We use `family = gaussian(link = "log")` to model the Geometric Mean of BPSysAve.
...
We now pass the fitted model to `plotint()`.

- `effect = "Age"`: continuous x-axis  
- `moderator = "BMI"`: continuous moderator  
- `mod_scale = "quantile"`: uses 10th, 50th, and 90th percentiles of BMI  
- `type = "response"`: plots the Geometric Mean SBP (back-transformed)  
- `pval_position = "bottom.right"`: displays the interaction p-value on the plot  
...
plot_data_list <- plotint(
  ...
  type = "response",            # Plot the Geometric Mean SBP
  show_ci = TRUE,
  bw = FALSE,
...
# The function also invisibly returns the plot's data
head(plot_data_list$plot_data)
```

The resulting plot shows the predicted geometric mean SBP on the y-axis and age on the x-axis. Each of the three lines represents a different level of BMI (10th, 50th, and 90th percentiles). The diverging lines and the significant p-value (P < 0.001) clearly indicate a statistical interaction: the effect of aging on blood pressure is more pronounced at higher levels of BMI.

## 8. Checking Proportional Hazards (Constant Effect) in Survival Models

A critical assumption in Cox survival models is that the effect of a predictor is constant over time (the Proportional Hazards or "PH" assumption). For complex survey data, the standard `survival::cox.zph()` function is not valid.

The `svyTable1` package provides two functions to correctly test this assumption by splitting the follow-up time into intervals, fitting a separate model to each interval, and plotting the coefficient to see if it remains constant.

- `svycoxph_CE()`: For standard `svydesign` objects.
- `svycoxph_CE_mi()`: For `svyimputationList` (multiple imputation) objects.

### Example 8A: The Standard Case (`svycoxph_CE`)
...
### Example 8B: The Multiple Imputation Case (`svycoxph_CE_mi`)
...
# 3. Create analysis dataframe and induce missingness
set.seed(123)
...
# 9. Run PH test function and print plot
