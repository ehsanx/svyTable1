...
1.  **Complex Survey Analysis:** Generating descriptive "Table 1" summaries (`svytable1()`), regression diagnostics (`svydiag()`), goodness-of-fit tests (`svygof()`), design-correct AUCs (`svyAUC()`) and analyzing interaction effects (`addint`, `addintlist`, `jointeffects`, `inteffects`) from **survey** package objects.
