...
## 5. Automated Three-Panel Report

The `reportint()` function is the recommended high-level wrapper for interaction analysis. It takes the *single* fitted interaction model (`interaction_model_logit`) and automatically generates all three standard report tables (Joint Effects, Simple Effects, and Additive Interaction) in one step. This is a significant simplification of the manual workflow shown in the following sections.

The function can return a list of tables (for use in vignettes or R scripts) or render a full, standalone HTML report directly to the RStudio Viewer.
